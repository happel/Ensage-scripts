--=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=---
--=========================INFO=======================    --
-- Scripts combined by Judge2020.                         --
-- I in no way, shape, or form made these scripts. All    --
-- I did was put them together.                           --
--                          V1                            --
--====================================================    --
--=========================TODO=======================    --
--1.Get buying items working. I have low lua knowledge    --
-- so some help would be greatly apreciated.              --
--2. Auto-change state of main meepo to farm jungle       --
--when the imeepo part of the script loads. Help here     --
--would also be nice.								      --
--3. any suggestions given may be taken into cinsideration--
...
config:SetParameter("Ult", 0) -- 1 = CD; 2 = none
config:Load()
local levels = {2,3,4,2,2,3,2,3,3,4,1,5,5,5,5,1,4,5,5,5,1,5,1,5,5}
local purchaseStartingItems = {39, 45, 16} -- Ring of regen, courier, branches
local BuyItem1 = {29,25,17,84} -- First buy
local BuyItem2 = {23,22,21,60}
local StepsOfBuy={BuyItem1,BuyItem2}
local StepsOfPrice = {1621,3001}
local buystate = 1
...
function Tick( tick )
	...
	if client.gameState == Client.STATE_PICK then 
		client:ExecuteCmd("dota_select_hero npc_dota_hero_meepo")
		currentLevel = 0
		buystate = 1
		TimeUseTree = 0
		NotFindTarget = 0
	...
	if PlayingGame() and me.alive then
		...
		BuyItems(me)
		if buystate >= 4 and buystate % 2 == 0 then 
